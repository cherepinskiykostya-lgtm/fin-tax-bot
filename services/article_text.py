...
def _first_nonempty_after(node: Node, headline: Node) -> Node | None:
...
def _iter_subtree_nodes(node: Node) -> Iterable[Node]:
    boundary = node.parent or node
    current: Node | None = node

    while current is not None:
        yield current
        current = _next_node_in_document(current, boundary)
...
def _collect_from_subtree(
    ...
    for current in _iter_subtree_nodes(node):
        if current.tag is None:
            continue
...
def _collect_after_headline(node: Node, headline: Node, seen: set[str]) -> list[str]:
    ...
    start = _first_nonempty_after(node, headline)
    if start is None:
        return blocks
    ...
    while current is not None:
        ...
        new_blocks, should_stop = _collect_from_subtree(current, seen, local_seen)
        if not skipped_date and not new_blocks and _DATE_PATTERN.search(text):
            skipped_date = True
            current = current.next
            continue
