...
WHITELIST_TAGS = {"p", "h2", "h3", "ul", "ol", "li", "blockquote", "table", "tr", "td"}
STOP_PHRASES = (
    "поділитися",
    "поширити",
    "останн",
...
def _remove_leading_noise(blocks: list[Block]) -> tuple[list[Block], int]:
    ...
    adjusted_paragraphs = sum(1 for block in blocks if block.tag == "p")
...
def _is_valid(text: str, paragraphs: int, html: str) -> bool:
    if not text:
        return False
    if len(text) < 400 and paragraphs < 2:
        return False
...
def _all_paragraphs(tree: HTMLParser) -> str | None:
    parts: list[str] = []
    seen: set[str] = set()
...
def extract_tax_body(html: str) -> Optional[str]:
    ...
    if fallback:
        log.info(
            "tax body: fell back to all <p> chars=%s paragraphs=%s",
            len(fallback),
            fallback.count("\n\n") + 1,
        )
        return fallback
