    ...
    async with SessionLocal() as s:  # type: AsyncSession
        rows = (
            await s.execute(
                select(Article)
                .where(
                    Article.level1_ok.is_(True),
                    or_(Article.taken.is_(False), Article.taken.is_(None)),
                )
...
async def _send_article_menu(update: Update, *, edit_message: bool = False):
    ...
    if edit_message and update.callback_query and update.callback_query.message:
        try:
            ...
            # Telegram не дозволяє редагувати повідомлення тим самим текстом —
            # у такому разі оновимо лише клавіатуру.
